...
$app_name = "";
$location_array = file("../data/locations.csv");//база maxmind - координаты русских городов
$nodes_array = file("../data/nodes.csv", FILE_IGNORE_NEW_LINES);//файл соответствия узла названию города
...
$filename = "../data/cache_data.txt";
$flag_old_data = false;

if (file_exists($filename)) {
	$handle = file("../data/cache_data.txt", FILE_IGNORE_NEW_LINES);

	/*$ldate = $handle[0];
	if ($ldate != date("Y-m-d h:i:s", $last_update[0]['lastclock']))
		$flag_old_data = true;*/

	if (!$flag_old_data)  $result_array = json_decode($handle[0], true);
    //echo "result: " . json_encode($handle, true);
} else {
  echo "Error! File cache_data.txt doesn`t exist.";
}
